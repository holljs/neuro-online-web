...
export default function NeuroArtist() {
  ...
  const [bonusClaimed, setBonusClaimed] = useState<boolean>(() => {
    ...
    if (savedId)
      return localStorage.getItem(`bonus_claimed_${savedId}`) === "true";
    return false;
  });
  ...
  useEffect(() => {
    if (userId) {
      ...
      axios
        .get(`${API_BASE}/bonus_status/${userId}`)
        .then((res) => {
          if (res.data.claimed) setBonusClaimed(true);
        })
        .catch(() => {});
    }
  }, [userId]);
  ...
  const refreshBalance = async () => {
    ...
    try {
      const res = await axios.get(
        `${API_BASE}/user/${userId}?_t=${Date.now()}`,
        {
  ...
  const loadLatestServerHistory = async () => {
    ...
    try {
      const res = await axios.get(
        `${API_BASE}/history/${userId}?limit=1&_t=${Date.now()}`,
      );
      if (res.data.success && res.data.items && res.data.items.length > 0) {
        ...
        const serverHistoryItem: HistoryItem = {
          ...
          date: lastItem.created_at
            ? new Date(lastItem.created_at).toLocaleDateString("ru-RU")
            : "Недавно",
          images: [],
          resultUrl: lastItem.result_url,
        };
        setHistory((prev) => {
          if (prev.length > 0 && prev[0].id === serverHistoryItem.id)
            return prev;
  ...
  const modesByCategory = {
    photo: [
      {
        ...
        name: "Люкс-Фото",
        cost: "4 кр.",
        desc: "Премиум генерация шедевров и графики с нуля или по фото",
      },
      {
        id: "vip_mix",
        name: "Реал-Фото",
        cost: "3 кр.",
        desc: "Точное сохранение лиц и внешности",
      },
      {
        id: "seadream_mix",
        name: "Пиар-Фото",
        cost: "4 кр.",
        desc: "Постеры, баннеры и рекламный дизайн",
      },
      {
        id: "ultra_photo",
        name: "Ультра-Реал",
        cost: "5 кр.",
        desc: "Премиум-реализм и русский текст",
      },
      {
        id: "gfpgan",
        name: "Реставратор",
        cost: "3 кр.",
        desc: "Повышение четкости и удаление шума",
      },
    ],
    video: [
      {
        ...
        name: "Живое Фото",
        cost: "3 кр.",
        desc: "Анимация и оживление портрета",
      },
      {
        id: "bytedance_5",
    ...
    audio: [
      ...
      },
      {
        ...
        name: "Озвучка текста",
        cost: "1 кр.",
        desc: "Превращение текста в красивую речь",
      },
    ],
    business: [
      {
        id: "wb_card",
        name: "Карточка WB / Ozon",
        cost: "4 кр.",
        desc: "Продающий баннер товара",
      },
      {
        id: "fashion",
        name: "Одежда на модели",
        cost: "4 кр.",
        desc: "Примерка на человека",
      },
      {
        id: "food",
        name: "Фуд-съемка",
        cost: "4 кр.",
        desc: "Мишлен-подача и коллажи",
      },
      {
        id: "furniture",
        name: "Интерьер и мебель",
        cost: "4 кр.",
        desc: "3D-визуализация комнат",
      },
    ],
  };
    ...
    try {
      ...
      try {
        ...
        const bridgeResponse: any = await Promise.race([
          ...
          new Promise((resolve) =>
            setTimeout(() => resolve({ result: true }), 2500),
          ),
        ]);
        if (bridgeResponse && bridgeResponse.result === false) {
          alert(
            "Вы отменили разрешение. Чтобы получить бонус, нужно разрешить сообщения от сообщества.",
          );
      ...
      if (e.response?.status === 400) {
        ...
        alert(
          "Ошибка: " +
            (e.response?.data?.detail || "Не удалось получить бонус"),
        );
      }
    } finally {
      setIsBonusLoading(false);
  ...
  const checkStatus = async (
    ...
    try {
      const res = await axios.get(
        `${API_BASE}/task_status/${taskId}?user_id=${userId}`,
        {
          headers: { "X-Bot-Token": BOT_TOKEN },
        },
      );
    ...
    try {
      ...
      alert(
        e.response?.data?.detail ||
          "Ошибка подключения к бэкенду или недостаточно кредитов.",
      );
      refreshBalance();
      setIsGenerating(false);
    }
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 bg-gray-100 dark:bg-gray-800 p-1.5 rounded-2xl">
            {[
              ...
              <button
                ...
                className={`py-2.5 px-3 text-xs sm:text-sm font-bold rounded-xl transition ${activeCategory === cat.id ? "bg-blue-600 text-white shadow-md" : "text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white"} cursor-pointer`}
              >
                {cat.label}
              </button>
            ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-6">
            {currentModes.map((mode) => (
              <button
                ...
                className={`p-4 rounded-xl border text-left transition ${activeMode === mode.id ? "border-blue-600 bg-blue-50/50 dark:bg-blue-900/20" : "border-gray-200 dark:border-gray-800 hover:border-gray-300"} cursor-pointer`}
              >
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900 space-y-4 shadow-sm">
              ...
              {selectedImages.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-3 items-center">
                  {selectedImages.map((src, index) => (
                    <div
                      key={index}
                      className="relative w-20 h-20 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm group"
                    >
                      <img
                        src={src}
                        alt="Загруженное"
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                <input
                  ...
                  value={stylePrompt}
                  onChange={(e) => setStylePrompt(e.target.value)}
                  placeholder="Свой стиль (например: Lo-Fi, female vocal, 120 bpm)"
                  className="w-full sm:w-1/2 rounded-xl border border-gray-300 dark:border-gray-700 bg-transparent p-3 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-blue-500"
                />
              </div>
          ...
          {activeMode !== "gfpgan" && (
            <div>
              ...
              <textarea
                ...
                className="w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-transparent p-4 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-blue-500 font-mono"
              />
            </div>
          )}
          <button
          ...
          >
            {isGenerating ? (
              <>
                <svg
                  className="spin-mini h-5 w-5 text-white"
            ...
            {isGenerating && (
              <div className="absolute inset-0 z-20 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm flex flex-col items-center justify-center p-4 sm:p-8 animate-pulse-slow">
                <div className="w-full max-w-sm h-48 sm:h-64 rounded-2xl animate-shimmer mb-4 sm:mb-6 border border-gray-100 dark:border-gray-800 flex items-center justify-center px-4">
                  <svg
                    className="w-12 h-12 sm:w-16 sm:h-16 text-blue-500 spin-mini"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
                ...
                <p className="text-xs sm:text-sm text-gray-500 max-w-xs sm:max-w-sm leading-normal">
                  Это может занять от пары секунд до 3 минут. Пожалуйста, не
                  закрывайте страницу.
                </p>
              </div>
            )}
              ...
              <>
                ...
                <div className="relative z-0 w-full">
                  {isVideoUrl(currentResultUrl) ? (
                    <video
                      ...
                      autoPlay
                      loop
                      className="mx-auto rounded-xl max-h-[500px] w-full object-contain shadow-lg"
                    />
                ...
                <div className="mt-5 relative z-0">
                  <a
                    ...
                    rel="noreferrer"
                    download
                    className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-blue-600 text-white font-bold text-xs hover:bg-blue-700 transition shadow-sm cursor-pointer"
                  >
      ...
      <div className="space-y-6">
        ...
        {userId && !bonusClaimed && (
          <div className="rounded-2xl border border-green-200 dark:border-green-900/30 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/10 p-5 shadow-sm space-y-3">
            ...
            <p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed">
              Разрешите рассылку от нашего сообщества — и получите{" "}
              <b className="text-green-600 dark:text-green-400">+3 кредита</b>{" "}
              на счёт бесплатно!
            </p>
            <button
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900 shadow-sm">
          <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
            ...
            <a
              href="/history"
              className="text-xs font-bold text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 transition cursor-pointer"
            >
              ...
              return (
                <div className="p-4 rounded-xl border border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/50 space-y-3 relative">
                  ...
                  <div className="space-y-1.5 bg-white dark:bg-gray-900 p-3 rounded-lg border border-gray-200/80 dark:border-gray-800">
                    <p
                      className={`text-xs text-gray-700 dark:text-gray-300 leading-relaxed font-medium ${!isPromptExpanded ? "line-clamp-3" : ""}`}
                    >
                      "{item.prompt}"
                    </p>
